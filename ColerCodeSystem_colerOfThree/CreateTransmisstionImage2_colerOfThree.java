 ...
 *
 * @see VisibleLightReceiver2_colerOfThree
 * @author Ogi
 * @version 1.0
 */
public class CreateTransmisstionImage2_colerOfThree extends Thread {
	...
	private String format = (Constants_colerOfThree.FORMAT);
	private String inputFileName = (Constants_colerOfThree.IN_IMG_PATH);
	private String outputFileName = (Constants_colerOfThree.OUT_IMG_PATH);
	...
	public CreateTransmisstionImage2_colerOfThree() {
		// Opencvの利用のため
		...
		// 各種画像を載せるパネル
		transmisstionImagePanel = new ImageDrawing_colerOfThree();
		...
		// 四隅のマーカ導入
		markerImage = Imgcodecs.imread(Constants_colerOfThree.MARK2);
		dimensionSetSize_Rows = (markerImage.rows() + Constants_colerOfThree.ROW_MARGIN);
		dimensionSetSize_cols = (markerImage.cols() + Constants_colerOfThree.COL_MARGIN);
		...
		if ((division = colorEncodeSize(outImgBytes)) != 0) {
			System.out.println("今回のブロックの数は" + (outImgBytes.length * Constants_colerOfThree.BLOCK_OF_BYTE) + "個なので" + division + "×"
					+ division + "のカラーコードのサイズに設定します");
		} else {
			System.out.println("サイズ設定でのエラーが発生したため、サイズの設定をキャンセルしました");
		}
//		 System.out.println(outImgBytes[0]);
...
//		 System.out.println((Integer.parseInt(Integer.toHexString((outImgBytes[0] & 0xff)))));
	 ...
	 * クラスの並列動作に利用<br>
	 *
	 * @see CreateTransmisstionImage2_colerOfThree#startRunning()
	 * @see CreateTransmisstionImage2_colerOfThree#stopRunning()
	 */
	public void run() {
		createImageLoop();
	}
	 ...
	 * 外部クラスから呼び出しクラスを実行する
	 *
	 * @see CreateTransmisstionImage2_colerOfThree#run()
	 * @see CreateTransmisstionImage2_colerOfThree#stopRunning()
	 */
	...
	private int colorEncodeSize(byte[] outBytes) {
		...
		codeSize: {
			for (int i = Constants_colerOfThree.BLOCL＿UNDER_LMIT; i <= Constants_colerOfThree.BLOCL＿TOP_LMIT; i++) {
				blocQuantity = (i * i);
				bytesLength = outBytes.length * Constants_colerOfThree.BLOCK_OF_BYTE;
				if (bytesLength < blocQuantity) {
					division = i;
					break codeSize;
				} else if (i > Constants_colerOfThree.BLOCL＿TOP_LMIT) {
	...
	private int[] colorEncodePattern(int byteIndex) {
		int colorEncodeOfBloc[] = new int[Constants_colerOfThree.BLOCK_OF_BYTE];
		int firstHalfFourBits = (Integer.parseInt(Integer.toHexString((outImgBytes[byteIndex] & 0xf0) >> 4),
				Constants_colerOfThree.HEXADECIMAL_NOTATION));
		...
		if (firstHalfFourBits > Constants_colerOfThree.COLLAR_VARIATION) {
			int a = firstHalfFourBits - Constants_colerOfThree.COLLAR_VARIATION;
			randmColorCordFirstHalfNumber = (int) (Math.random() * ((Constants_colerOfThree.COLLAR_VARIATION + 1) - a) + a);
		} else {
		}
		if (secondHalfFourBits > Constants_colerOfThree.COLLAR_VARIATION) {
			int b = secondHalfFourBits - Constants_colerOfThree.COLLAR_VARIATION;
			randmColorCordSecondHalfNumber = (int) (Math.random() * ((Constants_colerOfThree.COLLAR_VARIATION + 1) - b) + b);
		} else {
		}
		for (int i = 0; i <= Constants_colerOfThree.COLORENCODE_LOOP; i++) {
			int numberOfArrayElement;
			if (i == Constants_colerOfThree.COLORENCORD_BITS_FIRST_HALF) {
			...
			} else if (i == Constants_colerOfThree.COLORENCORD_BITS_SECOND_HALF) {
				numberOfArrayElement = i;
		...
		for (int i = 0; i < division; i++) {
			for (int j = 0; j < division; j++) {
				if (i == 0 && j == 0 || i == 0 && j == division - 1 || i == division - 1 && j == 0
					...
					if (count >= Constants_colerOfThree.BLOCK_OF_BYTE) {
						count = 0;
						byteIndexCounter++;
					}
					if (loopCount < outImgBytes.length * Constants_colerOfThree.BLOCK_OF_BYTE && count == 0) {
						// ここで一つのバイト配列の値から４つのブロックを決定
						colorEncodeOfBloc = colorEncodePattern(byteIndexCounter);
					} else if (loopCount < outImgBytes.length * Constants_colerOfThree.BLOCK_OF_BYTE && count != 0) {
					} else {
						for (int k = 0; k < colorEncodeOfBloc.length; k++) {
							colorEncodeOfBloc[k] = Constants_colerOfThree.COLORENCODE_SPACE;
						}
					}
					...
					Imgproc.rectangle(srcImage, new Point(x1, y1), new Point(x2, y2), paintColorBGR,
							Constants_colerOfThree.THICKNESS);

 ...
 * @version 1.0
 */
public class VisibleLightReceiver2_colerOfThree extends Thread {
	...
	private ImageDrawing_colerOfThree imageDrawing_colerOfThree;
	private ImageDrawing_colerOfThree processedImagePanel;
	private ImageDrawing_colerOfThree hsvImagePanel;
	private ImageDrawing_colerOfThree decordeImgPanel;
	...
	private CreateTransmisstionImage2_colerOfThree createTransmisstionImage2_colerOfThree;

	public VisibleLightReceiver2_colerOfThree(CreateTransmisstionImage2_colerOfThree createTransmisstionImage2_colerOfThree) {
		System.loadLibrary(Core.NATIVE_LIBRARY_NAME);// Opencvの利用のため
		captureCamera = new VideoCapture(0);// 使用webカメラの宣言
		this.createTransmisstionImage2_colerOfThree = createTransmisstionImage2_colerOfThree;
		// 加工画像用ウィンドウフレーム
		...
		processedImagePanel = new ImageDrawing_colerOfThree();
		decordeImgPanel = new ImageDrawing_colerOfThree();
		...
		hsvImagePanel = new ImageDrawing_colerOfThree();
		hsvImageFrame.setContentPane(hsvImagePanel);
		imageDrawing_colerOfThree = new ImageDrawing_colerOfThree();
		runningKey = false;
	 ...
	 * クラスの並列動作に利用<br>
	 *
	 * @see VisibleLightReceiver2_colerOfThree#startRunning()
	 * @see VisibleLightReceiver2_colerOfThree#stopRunning()
	 */
	public void run() {
		while (runningKey) {
			receiverLoop();
	 ...
	 * 外部クラスから呼び出しクラスを実行する
	 *
	 * @see VisibleLightReceiver2_colerOfThree#run()
	 * @see VisibleLightReceiver2_colerOfThree#stopRunning()
	 */
	...
	private Boolean rectangleChecker(MatOfPoint detectionContour, int areaThreshold) {
		if (detectionContour.total() != Constants_colerOfThree.SIDE_OF_THE_RECTANGLE) {// 辺の数の確認。矩形判断
			return false;
		}
	...
	private void colorDecorde(Mat srcImage, double startX, double startY, double endX, double endY, int division) {
		double[] data = new double[Constants_colerOfThree.HSV_CH];// HSV各チャンネル格納用
		double[] pointData = new double[Constants_colerOfThree.HSV_CH];// HSV各チャンネル格納用(取得)
		double[] averageData = new double[Constants_colerOfThree.HSV_CH];// HSV各チャンネル格納用(平均算出)
		...
		for (int i = 0; i < division; i++) {
			...
			for (int j = 0; j < division; j++) {
				...
				if (i == 0 && j == 0 || i == 0 && j == division - 1 || i == division - 1 && j == 0
					...
					for (int k = 0; k < Constants_colerOfThree.NUMBER＿OF_POINT; k++) {
						switch (k) {
						case 0:
						...
						case 1:
							...
							pointData=srcImage.get( y + Constants_colerOfThree.DISTANCE＿OF_POINT,x - Constants_colerOfThree.DISTANCE＿OF_POINT);
							Imgproc.circle(srcImage,
									new Point(x - Constants_colerOfThree.DISTANCE＿OF_POINT, y + Constants_colerOfThree.DISTANCE＿OF_POINT), 1,
									new Scalar(255, 255, 255), -1);// 色情報取得点可視化(まだ平均値を取得未実装)
					...
					for (int k = 0; k < averageData.length; k++) {
						data[k]=(averageData[k]/Constants_colerOfThree.NUMBER＿OF_POINT);//色相、彩度、明度それぞれの平均抽出
					}
					// H（色相）S(彩度) V(明度)を元に色を判断;
					// 8色用(白黒も入れて10色)メインで使用
					if (data[1] < 100 && data[2] >= 150) {
						receiveList.add("no");// 白
						hexadecimalOfBloc += Constants_colerOfThree.BLOC_COLLAR_OF_WHITE;
					} else if (data[2] < 100) {
						receiveList.add("space");// 黒
					} else if (data[1] >= 100 && data[0] * 2 >= 0 && data[0] * 2 < 20) {
						receiveList.add("1");// 赤
						hexadecimalOfBloc += Constants_colerOfThree.BLOC_COLLAR_OF_RED;
					} else if (data[1] >= 100 && data[0] * 2 >= 20 && data[0] * 2 < 35) {
						receiveList.add("7");// オレンジ
						hexadecimalOfBloc += Constants_colerOfThree.BLOC_COLLAR_OF_ORANGE;
					} else if (data[1] >= 100 && data[0] * 2 >= 35 && data[0] * 2 < 80) {
						receiveList.add("4");// 黄色
						hexadecimalOfBloc += Constants_colerOfThree.BLOC_COLLAR_OF_YELLOW;
					} else if (data[1] >= 100 && data[0] * 2 >= 80 && data[0] * 2 < 148) {
						receiveList.add("2");// 緑
						hexadecimalOfBloc += Constants_colerOfThree.BLOC_COLLAR_OF_GREEN;
					} else if (data[1] >= 100 && data[0] * 2 >= 148 && data[0] * 2 < 195) {
						receiveList.add("5");// 水色
						hexadecimalOfBloc += Constants_colerOfThree.BLOC_COLLAR_OF_CYAN;
					} else if (data[1] >= 100 && data[0] * 2 >= 195 && data[0] * 2 < 258) {
						receiveList.add("3");// 青
						hexadecimalOfBloc += Constants_colerOfThree.BLOC_COLLAR_OF_BULE;
					} else if (data[1] >= 100 && data[0] * 2 >= 258 && data[0] * 2 < 300) {
						receiveList.add("8");// 紫
						hexadecimalOfBloc += Constants_colerOfThree.BLOC_COLLAR_OF_PURPLE;
					} else if (data[1] >= 100 && data[0] * 2 >= 300 && data[0] * 2 < 330) {
						receiveList.add("6");// マゼンタ
						hexadecimalOfBloc += Constants_colerOfThree.BLOC_COLLAR_OF_MAGENTA;
					} else {
						receiveList.add("error");
						System.out.println("errorの数値\n" + data[0] * 2 + " " + data[1] + " " + data[2]);
					}
					//
					// if (blocDecordeCount <= Constants_colerOfThree.BLOCK_OF_BYTE) {
					...
					// Constants_colerOfThree.COLORENCORD_BITS_SECOND_HALF) {
					...
					blocDecordeCount++;
					// loopCount++;
					if (receiveList.isEmpty() || createTransmisstionImage2_colerOfThree.getTransmissionList().isEmpty()) {
						System.out.println("送受信が行われていません");
					} else if (createTransmisstionImage2_colerOfThree.getTransmissionList().size() != receiveList.size()) {
					...
					if (!receiveList.isEmpty() && !createTransmisstionImage2_colerOfThree.getTransmissionList().get(indexCounter)
							.equals(receiveList.get(indexCounter))) {
						missCount++;
						System.out.println(indexCounter + "つめの送信dataが"
								+ createTransmisstionImage2_colerOfThree.getTransmissionList().get(indexCounter) + "に対して"
	...
	private void receiverLoop() {
		...
		for (int i = 0; i < contoursList.size(); i++) {// 取得した輪郭の総数でループ
			if (hierarchyData.get(0, i)[3] == -1) {// 内部輪郭を持つ輪郭を弾く
				...
				if (markerChecker(contoursList.get(i), hsvImage, markerImage, 1000, division)) {// 輪郭が正方形であるかチェック
					...
					//Imgproc.medianBlur(markerImage, markerImage, 3);// 画像のノイズ処理→平滑化
					...
					BufferedImage bufferedImageTemp1 = imageDrawing_colerOfThree.matToBufferedImage(hImage);// 描画のためmat型からbufferedImage型に変換
		...
		BufferedImage bufferedImageTemp = imageDrawing_colerOfThree.matToBufferedImage(processedImage);// 描画のためmat型からbufferedImage型に変換
